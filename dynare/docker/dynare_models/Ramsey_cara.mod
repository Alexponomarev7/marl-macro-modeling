% Ramsey Model with CARA Utility and Population Growth
% 
% Timing convention:
...
%   5. Euler (CARA):         exp(-sigma*c_t) = beta * exp(-sigma*c_{t+1}) * (alpha*k_t^{alpha-1} + (1-delta) / (1+n))
%
% CARA utility: U(c) = 1 - exp(-sigma*c)
% Marginal utility: U'(c) = sigma * exp(-sigma*c)
...
[name='Euler equation CARA']
// MU(c_t) = beta * MU(c_{t+1}) * (MPK_{t+1} + 1 - delta)
// exp(-sigma*c_t) = beta * exp(-sigma*c_{t+1}) * (alpha*k_t^{alpha-1} + (1-delta) / (1+n))
exp(-sigma * ConsumptionPerCapita) = beta * exp(-sigma * ConsumptionPerCapita(+1)) * (alpha * CapitalPerCapita^(alpha-1) + (1-delta) / (1+n));
